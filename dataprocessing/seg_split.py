'''
dataset divide script  ver:  JAN 24th 2022 10: 00 unofficial release
    这个代码用作包含数据和对应掩码的数据集划分
    需要对应的data与mask重名，仅有后缀名的不同
...
def a_dataset_split(src_data_folder, class_name, train_scale, val_scale, test_scale, com_num=None):
    ...
    current_class_mask_path = os.path.join(r"C:\Users\admin\Desktop\MARS_Split\unsplit\mask", class_name)
    current_all_data = os.listdir(current_class_data_path)
    print(current_all_data)
    current_data_length = len(current_all_data)
    current_data_index_list = list(range(current_data_length))
    random.shuffle(current_data_index_list)

    train_folder = os.path.join(os.path.join(os.path.join(target_data_folder, 'train'), 'data'), class_name)
    val_folder = os.path.join(os.path.join(os.path.join(target_data_folder, 'val'), 'data'), class_name)
    test_folder = os.path.join(os.path.join(os.path.join(target_data_folder, 'test'), 'data'), class_name)
    ...
    for i in current_data_index_list:
        ...
        src_mask_path = os.path.join(current_class_mask_path, current_all_data[i].split(".")[0]+".png")
        # src_img_path&src_mask_path are bond with the same names of a pair of data&mask
        if current_idx <= train_stop_flag:
            copy2(src_img_path, train_folder)
            copy2(src_mask_path, train_mask_folder)
            print(src_mask_path)
            ...
            copy2(src_mask_path, val_mask_folder)
            # print("{} copied to{}".format(src_img_path, val_folder))
            val_num = val_num + 1

        else:
            copy2(src_img_path, test_folder)
            copy2(src_mask_path, test_mask_folder)
...
def data_set_split(src_data_folder, target_data_folder, train_scale=0.8, val_scale=0.2, test_scale=0.0,
    ...
    In the version of JAN 24th, this program divide 'data' first, than the 'mask' shall find its 'data' according to
    their names
    Make sure the source folder points to the folder containing the 'data'
    :param src_data_folder: source folder r'C:\Users\admin\Desktop\MARS_Split\unsplit\data'
    :param target_data_folder: target folder r'C:\Users\admin\Desktop\MARS_Split\split'
    ...
    type_names = ['data', 'mask']
    for split_name in split_names:
        split_path = os.path.join(target_data_folder, split_name)
        # Then create category folder under the split_path directory
        for type_name in type_names:
            type_split_path = os.path.join(split_path, type_name)
            for class_name in class_names:
                class_split_path = os.path.join(type_split_path, class_name)
...
if __name__ == '__main__':
    ...
    src_data_folder = r'C:\Users\admin\Desktop\MARS_Split\unsplit\data'
    target_data_folder = r'C:\Users\admin\Desktop\MARS_Split\split'
    data_set_split(src_data_folder, target_data_folder, train_scale=0.7, val_scale=0.1, test_scale=0.2)
